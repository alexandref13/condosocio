...
class Editais extends StatefulWidget {
  @override
  _EditaisState createState() => _EditaisState();
}

class _EditaisState extends State<Editais> {
  var editais = List<MapaDocumentos>();
  List<MapaDocumentos> _searchResult = [];
  bool isLoading = true;
  TextEditingController controller = TextEditingController();

  _getDocumentos() {
    ApiDocumentos.getDocumentosEdital().then((response) {
      setState(() {
        Iterable lista = json.decode(response.body);
        editais = lista.map((model) => MapaDocumentos.fromJson(model)).toList();
        isLoading = false;
      });
    });
  }
                                    ...
                                    child: ListTile(
                                      ...
                                      trailing: IconButton(
                                        ...
                                        onPressed: () => setState(
                                          () {
                                            _launched = _launchInBrowser(
                                                "https://condosocio.com.br/acond/downloads/documentos/${editais[index].imgdoc}");
                                          },
                                        ),
                                      ),
                                    ),
