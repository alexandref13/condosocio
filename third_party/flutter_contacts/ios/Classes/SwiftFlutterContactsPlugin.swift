...
public class SwiftFlutterContactsPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, CNContactViewControllerDelegate, CNContactPickerDelegate {
    ...
    private func present(
        ...
        guard let rootViewController = Self.resolveRootViewController() else {
            result(FlutterError(
                code: "no_root_view_controller",
                message: "flutter_contacts could not resolve a root view controller",
                details: nil
            ))
            return
        }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "openExternalViewOrEdit":
            DispatchQueue.main.async {
                ...
                if let firstContact = contacts.first {
                    let contactView = CNContactViewController(for: firstContact)
                    ...
                    self.present(navigationController, result: result)
                }
            }
        case "openExternalPick":
            DispatchQueue.main.async {
                ...
                self.present(contactPicker, result: result)
            }
        case "openExternalInsert":
            DispatchQueue.main.async {
                ...
                if let contactData = args?.first as? [String: Any?],
                    !contactData.isEmpty {
    ...
    @objc func contactViewControllerDidCancel() {
        if let result = externalResult {
            let viewController = Self.resolveRootViewController()?.presentedViewController
                ?? Self.resolveRootViewController()
            viewController?.dismiss(animated: true, completion: nil)
            result(nil)
            externalResult = nil
        }
